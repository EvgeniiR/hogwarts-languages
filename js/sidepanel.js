...
export function renderSide(){
  ...
  if(sTab==='vocab'){
    ...
    el.innerHTML=wk+items.slice().reverse().slice(0,20).map(m=>{
      ...
      return `<div class="mi"><div class="mi-row"><div><div class="mw">${esc(m.wrong)}</div><div class="mr">${esc(m.right)}</div><div class="mn">${esc(m.note||'')}</div></div><div class="vi-acts"><button class="vi-btn" onclick="editMistake(${idx})" aria-label="Editar"><i class="ti ti-pencil"></i></button><button class="vi-btn" onclick="deleteMistake(${idx})" aria-label="Eliminar"><i class="ti ti-trash"></i></button></div></div></div>`;
    }).join('');
  }
}
